# Prompt to Paste into Claude Code (VS Code / Cursor)

> Copy everything below the line and paste it as a single message in Claude Code.
> Make sure your workspace is rooted at `D:\Fuzebox\Fuzebox - AEOS - Details Interna\aeos-platform-main`.

---

You are helping me set up an **isolated research spike folder** inside the AEOS monorepo. This is a research-and-prototyping workstream — **DO NOT modify any existing production code, services, or configuration**. Everything you create must live inside one new folder.

## Hard Rules (non-negotiable)

1. **Read-only outside the spike folder.** You may read any file in the repo to understand context, but you may not modify, create, or delete files anywhere except inside the new folder `research/onet-spike/` (and you will create that folder).
2. **Do not add dependencies to root `package.json`, `pyproject.toml`, or any workspace package.** If the spike needs deps later, we'll discuss them separately.
3. **Do not modify the root `CLAUDE.md` or anything in `.claude/`.** Read them for context only.
4. **No code generation in this turn** — only folder + markdown scaffolding. Code comes in later sessions.
5. If anything below conflicts with conventions in the root `CLAUDE.md` or `.claude/` folder, **stop and ask me** before proceeding.

## Step 1 — Pre-Flight Reading (do this first, then summarize back to me)

Before creating anything, read and briefly summarize:

1. The root `CLAUDE.md` — what conventions does it establish? What's the agent's authority?
2. The `.claude/` folder contents — list every file and explain what each does (commands, agents, settings, hooks, MCP configs, etc.)
3. The `apps/` folder structure — what apps exist
4. The `services/` folder structure — confirm the six product services I expect: Assessment, Process Discovery, Telemetry, Intelligence, Recommendations, Governance
5. The `packages/canonical-schema/` package — this is the shared schema layer; understand its shape
6. The `sdk/adapters/` folder — what agent adapters already exist

After reading, give me a 10-line summary. Then **wait for my confirmation** before proceeding to Step 2.

## Step 2 — Create the Isolated Spike Folder

Once I confirm, create exactly this structure under the repo root:
...
Then add a single reference in the **root `CLAUDE.md`** under a new section called `## Active Research Spikes`:
...
This is the **only** modification to the root CLAUDE.md. Ask me before making it.

## Step 3 — Populate `research/onet-spike/CLAUDE.md`
...
## Step 4 — Populate `research/onet-spike/README.md`
...
After creating all of the above:

1. Show me the full tree of `research/onet-spike/`
2. Confirm no other files in the repo were modified (except the single import line in root `CLAUDE.md`, which you should have asked permission for in Step 2)
3. Tell me what you'd recommend tackling first in our next session

## Final reminder

Stay inside `research/onet-spike/`. Do not write code yet. Do not add dependencies. If you're unsure about anything in the existing AEOS conventions, stop and ask.
